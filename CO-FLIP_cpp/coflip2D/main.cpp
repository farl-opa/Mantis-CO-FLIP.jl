...
int main(int argc, char** argv)
{
    ...
    switch(experiment_name)
    {
        ...
        case 4:
        {
            ...
            // VTK exports for ParaView comparison
            std::string vtk_path = base_path + "/2D_convecting_vortex_periodic_res64_VTK/" + enumToString(sim_scheme) + "/";
            smokeSimulator.outputVelocityToVTK(vtk_path, filename, 0);
            smokeSimulator.outputVorticityToVTK(vtk_path, filename, 0);
            smokeSimulator.outputParticlesToVTK(vtk_path, filename, 0);
            ...
            for (int i = 0; i < total_frame; i++)
            {
                ...
                smokeSimulator.outputEnergy(filepath, curr_time);
                // VTK exports for ParaView comparison
                smokeSimulator.outputVelocityToVTK(vtk_path, filename, curr_i);
                smokeSimulator.outputVorticityToVTK(vtk_path, filename, curr_i);
                smokeSimulator.outputParticlesToVTK(vtk_path, filename, curr_i);
            }
        }
        break;
    }
