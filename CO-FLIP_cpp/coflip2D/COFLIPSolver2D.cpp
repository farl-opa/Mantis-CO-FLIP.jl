...
Vec2d sampleConvectingVortex(const Vec2d& pos, double U0, double Gamma, double sigma, const Vec2d& center) {
    Vec2d rel = pos - center;
    double r2 = rel[0] * rel[0] + rel[1] * rel[1];
    double sigma2 = sigma * sigma;
    double factor = (Gamma / (2.0 * M_PI)) * std::exp(-r2 / sigma2);
    double u_vortex = -factor * rel[1] / sigma2;
    double v_vortex =  factor * rel[0] / sigma2;
    return Vec2d(u_vortex + U0, v_vortex);
}
...
void COFLIPSolver2D::setInitVelocityConvectingVortex(double U0, double Gamma, double sigma, const Vec2d& center)
{
    if (sim_scheme != Scheme::CO_FLIP) {
        tbb::parallel_for(tbb::blocked_range2d<int,int>{0,ni+1,1,0,nj,1}, [&](const tbb::blocked_range2d<int,int> &r)
        {
            ...
            for (int j = r.cols().begin(); j < je; ++j) {
                for (int i = r.rows().begin(); i < ie; ++i) {
                    Vec2d pos = h * Vec2d(i, j) + h * Vec2d(0.0, 0.5);
                    u(i,j) = sampleConvectingVortex(pos, U0, Gamma, sigma, center)[0];
                }
            }
        });
        tbb::parallel_for(tbb::blocked_range2d<int,int>{0,ni,1,0,nj+1,1}, [&](const tbb::blocked_range2d<int,int> &r)
        {
            ...
            for (int j = r.cols().begin(); j < je; ++j) {
                for (int i = r.rows().begin(); i < ie; ++i) {
                    Vec2d pos = h * Vec2d(i, j) + h * Vec2d(0.5, 0.0);
                    v(i,j) = sampleConvectingVortex(pos, U0, Gamma, sigma, center)[1];
                }
            }
        });
    } else {
        ...
        tbb::parallel_for(tbb::blocked_range<int>(0,cParticles.size(),TBB_GRAINSIZE), [&](const tbb::blocked_range<int> &r)
        {
            for (int i = r.begin(); i < r.end(); ++i) {
                Vec2d pos = cParticles[i].pos_current;
                Vec2d result = sampleConvectingVortex(pos, U0, Gamma, sigma, center);
                cParticles[i].vel = result;
                cParticles[i].vel_temp = cParticles[i].vel;
            }
        });
...
void COFLIPSolver2D::outputVelocityToVTK(std::string folder, std::string filename_prefix, int frame)
{
    boost::filesystem::create_directories(folder);
    
    char framebuf[256];
    sprintf(framebuf, "%04d", frame);
    std::string filepath = folder + filename_prefix + "velocity_" + std::string(framebuf) + ".vti";
    ...
    // Write VTI header for cell-centered data
    fprintf(file, "<?xml version=\"1.0\"?>\n");
    fprintf(file, "<VTKFile type=\"ImageData\" version=\"0.1\" byte_order=\"LittleEndian\">\n");
    fprintf(file, "  <ImageData WholeExtent=\"0 %d 0 %d 0 0\" Origin=\"0 0 0\" Spacing=\"%f %f 1\">\n", 
            ni - 1, nj - 1, h, h);
    fprintf(file, "    <CellData Vectors=\"velocity\">\n");
    fprintf(file, "      <DataArray type=\"Float32\" Name=\"velocity\" NumberOfComponents=\"3\" format=\"ascii\">\n");

    // Sample velocity at cell centers (simple approach: use cell-centered averages)
    for (int j = 0; j < nj; ++j) {
        for (int i = 0; i < ni; ++i) {
            // Average u components
            double u_left  = (i > 0) ? u(i-1, j) : u(i, j);
            double u_right = (i < ni) ? u(i, j) : u(i-1, j);
            double vel_u = 0.5 * (u_left + u_right);
            
            // Average v components
            double v_bottom = (j > 0) ? v(i, j-1) : v(i, j);
            double v_top    = (j < nj) ? v(i, j) : v(i, j-1);
            double vel_v = 0.5 * (v_bottom + v_top);
    ...
    fclose(file);
    std::cout << "Saved velocity field to: " << filepath << std::endl;
}

void COFLIPSolver2D::outputVorticityToVTK(std::string folder, std::string filename_prefix, int frame)
{
    boost::filesystem::create_directories(folder);
    
    char framebuf[256];
    sprintf(framebuf, "%04d", frame);
    std::string filepath = folder + filename_prefix + "vorticity_" + std::string(framebuf) + ".vti";

    calculateCurl(false, false);
    ...
    fprintf(file, "  <ImageData WholeExtent=\"0 %d 0 %d 0 0\" Origin=\"0 0 0\" Spacing=\"%f %f 1\">\n", 
            ni - 1, nj - 1, h, h);
    fprintf(file, "    <CellData Scalars=\"vorticity\">\n");
    fprintf(file, "      <DataArray type=\"Float32\" Name=\"vorticity\" format=\"ascii\">\n");

    // Write vorticity at cell centers
    for (int j = 0; j < nj; ++j) {
        for (int i = 0; i < ni; ++i) {
            double vort = curl(i, j);
            fprintf(file, "%f\n", vort);
        }
    }
    ...
    fclose(file);
    std::cout << "Saved vorticity field to: " << filepath << std::endl;
}

void COFLIPSolver2D::outputParticlesToVTK(std::string folder, std::string filename_prefix, int frame)
{
    boost::filesystem::create_directories(folder);
    
    char framebuf[256];
    sprintf(framebuf, "%04d", frame);
    std::string filepath = folder + filename_prefix + "particles_" + std::string(framebuf) + ".vtu";
    ...
    int num_particles = (int)cParticles.size();

    // Write VTU header (unstructured grid)
    fprintf(file, "<?xml version=\"1.0\"?>\n");
    ...
    fprintf(file, "      <Points>\n");
    fprintf(file, "        <DataArray type=\"Float32\" NumberOfComponents=\"3\" Name=\"Points\" format=\"ascii\">\n");
    for (int i = 0; i < num_particles; ++i) {
        fprintf(file, "%f %f 0\n", cParticles[i].pos_current[0], cParticles[i].pos_current[1]);
    }
    fprintf(file, "        </DataArray>\n");
    fprintf(file, "      </Points>\n");
    ...
    // Point Data
    fprintf(file, "      <PointData Vectors=\"velocity\" Scalars=\"volume\">\n");
    
    // Velocity
    fprintf(file, "        <DataArray type=\"Float32\" Name=\"velocity\" NumberOfComponents=\"3\" format=\"ascii\">\n");
    for (int i = 0; i < num_particles; ++i) {
        fprintf(file, "%f %f 0\n", cParticles[i].vel[0], cParticles[i].vel[1]);
    }
    fprintf(file, "        </DataArray>\n");

    // Volume
    fprintf(file, "        <DataArray type=\"Float32\" Name=\"volume\" format=\"ascii\">\n");
    for (int i = 0; i < num_particles; ++i) {
        fprintf(file, "%f\n", cParticles[i].volume);
    }
    fprintf(file, "        </DataArray>\n");
    ...
    fclose(file);
    std::cout << "Saved particles to: " << filepath << std::endl;
}
