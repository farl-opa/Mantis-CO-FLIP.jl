...
public:
    ...
    void outputVelocityToVTK(std::string folder, std::string filename_prefix, int frame);
    void outputVorticityToVTK(std::string folder, std::string filename_prefix, int frame);
    void outputParticlesToVTK(std::string folder, std::string filename_prefix, int frame);
    void initSmokePlume();
    ...
    void applyPeriodicWrapping(double L) {
        if (!do_periodic) return;
        tbb::parallel_for(tbb::blocked_range<int>(0, cParticles.size(), TBB_GRAINSIZE), 
            [&](const tbb::blocked_range<int> &r) {
                for (int p = r.begin(); p < r.end(); ++p) {
                    cParticles[p].pos_current = wrapPeriodicPosition(cParticles[p].pos_current, L);
                    cParticles[p].pos_temp = wrapPeriodicPosition(cParticles[p].pos_temp, L);
                }
            });
    }
    ...
    inline Vec2d wrapPeriodicPosition(const Vec2d& pos, double L) const {
        if (!do_periodic) return pos;
        Vec2d wrapped = pos;
        while (wrapped[0] < 0) wrapped[0] += L;
        while (wrapped[0] > L) wrapped[0] -= L;
        while (wrapped[1] < 0) wrapped[1] += L;
        while (wrapped[1] > L) wrapped[1] -= L;
        return wrapped;
    }
private:
    double m_max_curl = 0.;
