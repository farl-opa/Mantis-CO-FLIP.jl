...
void writeBMP(const char* filename, unsigned int w, unsigned int h, double *pixels) {
	FILE *f;
	unsigned char *img = NULL;
	int filesize = 54 + 3 * w*h;  //w is your image width, h is image height, both int
...
void writeBMPColor(const char* filename, unsigned int w, unsigned int h, double *first, double *second) {
    FILE *f;
    unsigned char *img = NULL;
    int filesize = 54 + 3 * w*h;  //w is your image width, h is image height, both int
...
void writeBMPColorVector(const char* filename, unsigned int w, unsigned int h, double *pixels) {
    FILE *f;
    unsigned char *img = NULL;
    int filesize = 54 + 3 * w*h;  //w is your image width, h is image height, both int
    ...
    for (int i = 0; i<w; i++)
    {
        for (int j = 0; j<h; j++)
        {
            int x = i; int y = (h - 1) - j;
            int idx = (j*w + i)*3;
            if (pixels[idx+0] < 0) pixels[idx+0] = 0;
            if (pixels[idx+0] > 1) pixels[idx+0] = 1;
            if (pixels[idx+1] < 0) pixels[idx+1] = 0;
            if (pixels[idx+1] > 1) pixels[idx+1] = 1;
            if (pixels[idx+2] < 0) pixels[idx+2] = 0;
            if (pixels[idx+2] > 1) pixels[idx+2] = 1;
            unsigned char r = pixels[idx+0] * 255;
            unsigned char g = pixels[idx+1] * 255;
            unsigned char b = pixels[idx+2] * 255;
            img[(x + y*w) * 3 + 2] = (unsigned char)(r);
            img[(x + y*w) * 3 + 1] = (unsigned char)(g);
            img[(x + y*w) * 3 + 0] = (unsigned char)(b);
        }
