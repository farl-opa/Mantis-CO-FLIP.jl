...
struct _interpreter {
    ...
    PyObject *s_python_function_colorbarsimple;
...
#  endif
    ...
    _interpreter() {
        ...
        s_python_function_colorbarsimple = safe_import(pymod, "colorbar");
...
template<typename NumericX, typename NumericY, typename NumericColors>
                 ...
                 const std::map<std::string, std::string> & keywords = {})
    {
        ...
        for (const auto& it : keywords)
        {
            if (it.first == "linewidths" || it.first == "alpha" || it.first == "vmin" || it.first == "vmax") {
                PyDict_SetItemString(kwargs, it.first.c_str(),
                PyFloat_FromDouble(std::stod(it.second)));
            } else {
                PyDict_SetItemString(kwargs, it.first.c_str(),
...
inline void colorbarSimple()
{
    detail::_interpreter::get();

    PyObject* res = PyObject_Call(detail::_interpreter::get().s_python_function_colorbarsimple, 
        detail::_interpreter::get().s_python_empty_tuple, detail::_interpreter::get().s_python_empty_tuple);
...
inline void figure_size(size_t w, size_t h, size_t dpi)
{
    detail::_interpreter::get();
