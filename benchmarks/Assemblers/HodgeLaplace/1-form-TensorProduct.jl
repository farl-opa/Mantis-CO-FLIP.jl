...
function problem_setup(manifold_dim::Int, num_els::Int, p::Int, k::Int)
    ...
    _, _, fₑ = sinusoidal_data(1, Mantis.Forms.get_geometry(ℜ⁰))
...
function run_problem(manifold_dim::Int, num_els::Int, p::Int, k::Int)
    ...
    weak_form_inputs = Assemblers.WeakFormInputs((ℜ⁰, ℜ¹), (fₑ,))
    lhs_expressions, rhs_expressions = Assemblers.one_form_hodge_laplacian(
...
manifold_dim = [2, 3]
num_elements = [2 .^ (1:3), 2 .^ (1:2)] # For each manifold_dim
p = 3
k = p - 1
