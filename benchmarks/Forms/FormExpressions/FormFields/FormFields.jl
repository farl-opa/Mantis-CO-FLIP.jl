module EvaluateFormFieldsBenchmarks
...
function run_problem(::Val{manifold_dim}, ::Val{form_rank}) where {manifold_dim, form_rank}
    ...
    space = R[form_rank + 1]
    field = Forms.FormField(space, rand(Forms.get_num_basis(space)), "test")
    eval_points = Points.CartesianPoints(
        ntuple(manifold_dim) do m
            return LinRange(0.0, 1.0, 10)
        end,
    )
    return @benchmarkable Forms.evaluate($field, 5, $eval_points) samples = 500 evals = 1 seconds =
        Inf
end
