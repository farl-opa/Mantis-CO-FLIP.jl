module ExteriorDerivativeBenchmarks
...
mod = include("ExteriorDerivative.jl")
sub_group = getproperty(mod, :group)
group["ExteriorDerivative"] = sub_group
...
if show || save
    for (file_name, benchmark) in group["ExteriorDerivative"]
