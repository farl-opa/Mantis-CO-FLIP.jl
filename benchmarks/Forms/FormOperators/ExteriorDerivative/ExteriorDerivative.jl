module EvaluateExteriorDerivativeBenchmarks
...
function run_problem(::Val{manifold_dim}, ::Val{form_rank}) where {manifold_dim, form_rank}
    ...
    exterior_derivative = Forms.ExteriorDerivative(R[form_rank + 1])
    eval_points = Points.CartesianPoints(
        ntuple(manifold_dim) do m
            return LinRange(0.0, 1.0, 10)
        end,
    )
    return @benchmarkable Forms.evaluate($exterior_derivative, 5, $eval_points) samples =
        500 evals = 1 seconds = Inf
end
