...
for geom in [geo_2d_cart, tensor_prod_geo, geom_crazy]
    ...
    for elem_id in 1:1:Mantis.Geometry.get_num_elements(geom)
        ...
        # 1-forms
        println(Mantis.Forms.evaluate(★ζ¹, elem_id, Mantis.Quadrature.get_quadrature_nodes(q_rule)))
        Mantis.Forms.evaluate(★ζ¹, elem_id, Mantis.Quadrature.get_quadrature_nodes(q_rule))
        @test all(isapprox(Mantis.Forms.evaluate(★ζ¹, elem_id, Mantis.Quadrature.get_quadrature_nodes(q_rule))[1][1], 1.0./det_g, atol=1e-12))
