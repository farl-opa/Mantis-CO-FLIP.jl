...
# curvilinear mesh
crazy_c = 0.2
function mapping_ip_test(x::AbstractVector)
...
function dmapping_ip_test(x::AbstractVector)
    x1_new = (2.0/(Lright-Lleft))*x[1] - 2.0*Lleft/(Lright-Lleft) - 1.0
    x2_new = (2.0/(Ltop-Lbottom))*x[2] - 2.0*Lbottom/(Ltop-Lbottom) - 1.0
...
for geom in [geom_cart, geom_crazy]
    ...
    for elem_id in 1:1:Mantis.Geometry.get_num_elements(geom)
        ...
        for node_idx in eachindex(Mantis.Quadrature.get_weights(q_rule))
            integrated_metric_1 .+= Mantis.Quadrature.get_weights(q_rule)[node_idx] .* ((inv_g[node_idx,:,:]).*det_g[node_idx])
        end
