...
for geom in [geo_2d_cart, tensor_prod_geo, geom_crazy]
    ...
    for elem_id in 1:1:Mantis.Geometry.get_num_elements(geom)
        ...
        hodge_zero_form_eval, hodge_zero_form_indices = Mantis.Forms.evaluate(★α⁰, elem_id, Mantis.Quadrature.get_nodes(q_rule))
        ...
        zeta_one_form_eval, zeta_one_form_indices = Mantis.Forms.evaluate(ζ¹, elem_id, Mantis.Quadrature.get_nodes(q_rule))
        ...
        hodge_hodge_1_eval, hodge_hodge_1_indices = Mantis.Forms.evaluate(★★ζ¹, elem_id, Mantis.Quadrature.get_nodes(q_rule))
        ...
        hodge_top_eval, hodge_top_indices = Mantis.Forms.evaluate(★γ², elem_id, Mantis.Quadrature.get_nodes(q_rule))
        top_eval, top_indices = Mantis.Forms.evaluate(γ², elem_id, Mantis.Quadrature.get_nodes(q_rule))
...
for geom in [geo_3d_cart, crazy_geo_3d_cart]
    ...
    for elem_id in 1:Mantis.Geometry.get_num_elements(geom)
        ...
        # 0-forms
        # Hodge of a unity 0-form is the volume form and has only 1 component.
        hodge_0_form_eval, hodge_alpha_indices = Mantis.Forms.evaluate(★α⁰, elem_id, Mantis.Quadrature.get_nodes(q_rule))
        ...
        hodge_1_eval, hodge_1_indices = Mantis.Forms.evaluate(★ζ¹, elem_id, Mantis.Quadrature.get_nodes(q_rule))
        form_zeta_eval, form_zeta_indices = Mantis.Forms.evaluate(ζ¹, elem_id, Mantis.Quadrature.get_nodes(q_rule))
        ...
        hodge_2_eval, hodge_2_indices = Mantis.Forms.evaluate(★ζ², elem_id, Mantis.Quadrature.get_nodes(q_rule))
        zeta_eval, zeta_indices = Mantis.Forms.evaluate(ζ², elem_id, Mantis.Quadrature.get_nodes(q_rule))
        ...
        # Finally, test if the Hodge-⋆ is the inverse of itself (in 3D without minus signs needed)
        hodge_hodge_2_eval, hodge_hodge_2_indices = Mantis.Forms.evaluate(★★ζ², elem_id, Mantis.Quadrature.get_nodes(q_rule))
        zeta_eval, zeta_indices = Mantis.Forms.evaluate(ζ², elem_id, Mantis.Quadrature.get_nodes(q_rule))
    ...
    #                          form_eval[3] .* (inv_g[:, 1, 1] .* inv_g[:, 2, 2] - inv_g[:, 1, 2] .* inv_g[:, 2, 1])) .* sqrt_g
        ...
        # n-forms
        # Hodge of a unity n-form is a form and has only 1 component.
        hodge_top_form_eval, hodge_top_form_indices = Mantis.Forms.evaluate(★γ³, elem_id, Mantis.Quadrature.get_nodes(q_rule))
        top_form_eval, top_form_indices = Mantis.Forms.evaluate(γ³, elem_id, Mantis.Quadrature.get_nodes(q_rule))
