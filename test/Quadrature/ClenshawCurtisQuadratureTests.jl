...
# The degree used for the Clenshaw-Curtis quadrature rule does not 
...
for p in range(2, 20, step=3)
    ...
    @test Mantis.Quadrature.get_quadrature_rule_type(quad_rule) == "Clenshaw-Curtis"
    ...
    # up to p (with p the degree, meaning that  Clenshaw-Curtis 
    # quadrature will use p+1 nodes).
    for degree in 0:p
        f = monomial(degree, ξ)
    ...
    # The monomials can be exactly integrated for degrees up to 2p. 
    ...
    # However, Clenshaw-Curtis quadrature is exact for Chebyshev 
    # polynomials when p is even, see Trefethen2022.
    f = chebyshev(p+1, ξ)
    I_num = LinearAlgebra.dot(w, f)
