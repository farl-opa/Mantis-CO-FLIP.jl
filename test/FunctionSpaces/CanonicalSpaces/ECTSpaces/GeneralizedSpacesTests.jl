...
# Test the GeneralizedTrigonometric space on [0, 1] ---------------------------
...
# Test the GeneralizedTrigonometric space on [0, 0.5] ---------------------------
...
# Test the GeneralizedExponential space on [0, 1] ---------------------------
...
for p in degrees_to_test
    ...
    b = Mantis.FunctionSpaces.GeneralizedExponential(p, Wt)
    b_eval = Mantis.FunctionSpaces.evaluate(b, x, 1)
    ...
    if p > 2
        ...
        @test isapprox(maximum(abs.(b_eval[1][1] * coeff_b .- f_eval)), 0.0, atol = 1e-14)
        # ... the first order derivative matches df/dx ...
        @test isapprox(maximum(abs.(b_eval[2][1] * coeff_b .- df_dx_eval)), 0.0, atol = 1e-11)
        # ... and the second order derivative matches d2f/dx2.
        @test isapprox(maximum(abs.(b_eval[3][1] * coeff_b .- d2f_dx2_eval)), 0.0, atol = 2e-10)
    end
end

# Test the GeneralizedExponential space on [0, 0.5] ---------------------------
...
for p in degrees_to_test
    ...
    # Zero sum of derivatives
    @test all(isapprox.(abs.(sum(b_eval[2][1], dims=2)), 0.0, atol=2e-12))
    ...
    if p > 2
        ...
        @test isapprox(maximum(abs.(b_eval[2][1] * coeff_b / L .- df_dx_eval)), 0.0, atol = 1e-11)
        # ... and the second order derivative matches d2f/dx2.
        @test isapprox(maximum(abs.(b_eval[3][1] * coeff_b / L^2 .- d2f_dx2_eval)), 0.0, atol = 2e-10)
    end
end
