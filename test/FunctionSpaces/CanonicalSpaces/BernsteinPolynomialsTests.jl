...
for p in degrees_to_test
    ...
    if p > 0
        ...
        f =
            (x::Float64, p::Int64, m::Int64) ->
                (p - m >= 0 ? 1.0 : 0.0) *
                prod(LinRange(p:-1:(p - m + 1))) *
                (p - m > 0 ? x^(p - m) : 1.0) +
                (p - 1 - m >= 0 ? 1.0 : 0.0) *
        ...
        coeff_b = FunctionSpaces.extract_monomial_to_bernstein(b) * coeff_m
        # Check that the values match f ...
        @test isapprox(maximum(abs.(b_eval[1][1] * coeff_b .- f_eval)), 0.0, atol=1e-15)
        # ... the first order derivative matches df/dx ...
        @test isapprox(maximum(abs.(b_eval[2][1] * coeff_b .- df_dx_eval)), 0.0, atol=2e-14)
        # ... and the second order derivative matches d2f/dx2.
        if p > 1
            @test isapprox(
                maximum(abs.(b_eval[3][1] * coeff_b .- d2f_dx2_eval)), 0.0, atol=2e-13
            )
        end
    end
end
