...
for deg1 in 0:5
    for deg2 in 0:5
        ...
        local B1 = Mantis.FunctionSpaces.BSplineSpace(
            patch1, deg1, [-1, deg1 - 1, min(deg1 - 1, 0), deg1 - 1, -1]
        )
        # second B-spline patch
        local B2 = Mantis.FunctionSpaces.BSplineSpace(
            patch2, deg2, [-1, min(deg2 - 1, 1), deg2 - 1, -1]
        )
        # tensor-product B-spline patch
        TP = Mantis.FunctionSpaces.TensorProductSpace((B1, B2))
        TP1 = Mantis.FunctionSpaces.TensorProductSpace((
            Mantis.FunctionSpaces.TensorProductSpace((B1, B2)), B1
        ))
        TP2 = Mantis.FunctionSpaces.TensorProductSpace((
            B1, Mantis.FunctionSpaces.TensorProductSpace((B2, B1))
        ))
        ...
        qrule3 = Mantis.Quadrature.tensor_product_rule(
            (deg1 + 1, deg2 + 1, deg1 + 1), Mantis.Quadrature.gauss_legendre
        )
        ...
        @test Mantis.FunctionSpaces.get_num_basis(TP1) ==
            Mantis.FunctionSpaces.get_num_basis(TP2)
        @test Mantis.FunctionSpaces.get_num_basis(TP2) ==
            Mantis.FunctionSpaces.get_num_basis(TP3)

        # tests for dof partitioning
        @test Mantis.FunctionSpaces.get_dof_partition(TP1) ==
            Mantis.FunctionSpaces.get_dof_partition(TP2)
        @test Mantis.FunctionSpaces.get_dof_partition(TP2) ==
            Mantis.FunctionSpaces.get_dof_partition(TP3)
        @test sum(length, Mantis.FunctionSpaces.get_dof_partition(TP3)[1]) ==
            Mantis.FunctionSpaces.get_num_basis(TP3)
    end
end
...
# second B-spline patch
deg2 = 4
B2_tensor_prod = Mantis.FunctionSpaces.BSplineSpace(
    patch2, deg2, [-1, min(deg2 - 1, 1), deg2 - 1, -1]
)
...
# second B-spline patch
deg2 = 3
B4_tensor_prod = Mantis.FunctionSpaces.BSplineSpace(
    patch2, deg2, [-1, min(deg2 - 1, 1), deg2 - 1, -1]
)
# First multi-patch object
