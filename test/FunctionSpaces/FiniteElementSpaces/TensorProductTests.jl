...
for deg1 in 0:5
    for deg2 in 0:5
        ...
        for el in 1:1:Mantis.FunctionSpaces.get_num_elements(TP)
            ...
            @test minimum(TP1_eval[1][1]) >= 0.0
            @test minimum(TP2_eval[1][1]) >= 0.0
            @test minimum(TP3_eval[1][1]) >= 0.0

            # Partition of unity
            @test all(isapprox.(sum(TP_eval[1][1], dims=2), 1.0))
            @test all(isapprox.(sum(TP1_eval[1][1], dims=2), 1.0))
            @test all(isapprox.(sum(TP2_eval[1][1], dims=2), 1.0))
            @test all(isapprox.(sum(TP3_eval[1][1], dims=2), 1.0))
