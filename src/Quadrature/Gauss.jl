...
- `::QuadratureRule{1}`: 1 dimensional quadrature rule containing the 
                         nodes and weights. There will be `N` nodes and
                         weights.

# Throws
- `DomainError`: If `N` is less than or equal to 1. This is handled by 
                 the FastGaussQuadrature.jl package.
...
function gauss_legendre(N::Int)
    if N <= 0
        throw(DomainError("Invalid number of nodes: $N. Gauss-Legendre quadrature must have at least one node."))
    end
