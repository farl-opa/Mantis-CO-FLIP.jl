...
A multi-valued space that is the direct sum of `num_spaces` multi-valued function spaces.
Consequently, their basis functions are evaluated independently and arranged in a
block-diagonal matrix. Each multi-valued function space contributes to several component of
the multi-valued space. If `num_spaces` is equal to `num_components`, then the
CompositeDirectSumSpace is equivalent to a DirectSumSpace.

# Fields
- `component_spaces::F`: Tuple of `num_components` scalar function spaces
"""
struct CompositeDirectSumSpace{manifold_dim, num_spaces, num_components, F} <:
       AbstractFESpace{manifold_dim, num_components}
    component_spaces::F
    component_ordering::NTuple{num_spaces, Vector{Int}}

    function CompositeDirectSumSpace(
        component_spaces::F, component_ordering::NTuple{num_spaces, Vector{Int}}
    ) where {num_spaces, F <: NTuple{num_spaces, AbstractFESpace}}
        manifold_dim = get_manifold_dim(component_spaces[1])
        num_components = 0
        for space in component_spaces
            if get_manifold_dim(space) != manifold_dim
                throw(ArgumentError(
                        "All component spaces must have the same manifold dimension"
                ))
            end
        ...
        if num_components != maximum(tmp)
            throw(ArgumentError("""\
                The number of components does not correspond to the component ordering \
                provided.\
                """
            ))
        end
        ...
        return new{manifold_dim, num_spaces, num_components, F}(
            component_spaces, component_ordering
        )
    end
    ...
    function CompositeDirectSumSpace(
        ...
        component_ordering = Tuple(
            collect((component_offsets[i] + 1):component_offsets[i + 1]) for
            i in 1:num_spaces
        )

        return CompositeDirectSumSpace(component_spaces, component_ordering)
    end
end

"""
    get_component_spaces(space::CompositeDirectSumSpace)

Get the component spaces of the composite direct sum space.

# Arguments
- `space::CompositeDirectSumSpace`: Direct-sum space

# Returns
- `component_spaces::F`: Tuple of component spaces
"""
function get_component_spaces(space::CompositeDirectSumSpace)
    return space.component_spaces
end

"""
    evaluate(
        space::CompositeDirectSumSpace{manifold_dim, num_spaces, num_components, F},
        element_idx::Int,
        xi::NTuple{manifold_dim, Vector{Float64}},
        nderivatives::Int,
    ) where {manifold_dim, num_spaces, num_components, F <: NTuple{num_spaces, AbstractFESpace}}

Evaluate the basis functions of the composite direct sum space at the points `xi` in the
element with index `element_idx`. The function returns a tuple of `num_components` arrays,
built from the evaluations of the basis functions of the component space.

# Arguments
- `space::CompositeDirectSumSpace{manifold_dim,num_spaces,num_components,F}`: Direct sum
    space
...
# Returns
- `local_multivalued_basis::Vector{Vector{Vector{Array{Float64, 2}}}}`: Matrices containing
    the evaluations of the basis functions and its derivatives of the direct sum space.
    `local_multivalued_basis[j][k][i][l, m]` contains the (j-1)th-order derivative, with
    respect to the k-th coordinate, of the m-th multivalued basis of component i evaluated
    at the lth-point. In this case the maximum order of derivative is first order. For
    higher order derivatives we should follow a flattenned numbering using the indices of
    the derivatives.
...
function evaluate(
    ...
    multivalued_basis_indices, component_space_basis_indices = get_basis_indices_w_components(
        space, element_idx
    )
    ...
    for space_idx in 1:num_spaces
        ...
        local_space_basis, _ = FunctionSpaces.evaluate(
            space.component_spaces[space_idx], element_idx, xi, nderivatives
        )
        ...
        for key in der_keys
            ...
            for i in 1:num_components_per_space[space_idx]
                ...
                # the `i`-th local component is stored as the `component_idx`-th component
                # of the multi-valued space
...
function get_num_basis(space::CompositeDirectSumSpace, element_id::Int)
    return sum(get_num_basis.(space.component_spaces, element_id))
end

function get_basis_indices(
    space::CompositeDirectSumSpace{manifold_dim, num_spaces, num_components, F},
    element_idx::Int,
) where {manifold_dim, num_spaces, num_components, F}
    component_space_basis_indices =
...
"""
    get_basis_indices_w_components(
        space::CompositeDirectSumSpace{manifold_dim, num_spaces, num_components, F},
        element_idx::Int,
    ) where {manifold_dim, num_spaces, num_components, F}

Get the global indices of the multivalued basis functions of the direct sum space as well
as the component spaces for the element with index `element_idx`.

# Arguments
- `space::CompositeDirectSumSpace{manifold_dim, num_spaces, num_components, F}`: Composite
    direct sum space.
- `element_idx::Int`: Index of the element
...
function get_max_local_dim(space::CompositeDirectSumSpace)
    max_local_dim = 0
