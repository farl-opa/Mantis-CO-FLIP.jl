...
function PolarSplineSpace(
    ...
    if form_rank > 0
        if isnothing(dspace_p) || isnothing(dspace_r)
            throw(ArgumentError("""\
                Derivative spaces are required for form_rank > 0 but haven't been provided.\
                """
            ))
        end
    end
    ...
    E, control_triangle = build_polar_extraction_operators(
        singularity_type, degenerate_control_points, n_p, n_r; form_rank=form_rank
    )
    E = E[form_rank + 1]
    ...
    for component_idx in eachindex(E)
        # number of elements
        ...
        for i in 1:num_elements
            ...
            extraction_coefficients[count + 1] =
                Array(E[component_idx][basis_indices[count + 1], cols_ij])'
            count += 1
        end
        ...
        polar_splines[component_idx] = UnstructuredSpace(
            (tp_space[component_idx],),
            extraction_op,
            dof_partition,
            us_config,
...
"""
    build_polar_extraction_operators(
        polar_config::Int,
        degenerate_control_points::NTuple{2, Matrix{Float64}},
        num_basis_p::Int,
        num_basis_r::Int;
        form_rank::Union{Nothing, Int}=nothing,
    )

Build the extraction operators for the polar spline space. The extraction operators are
used to extract the polar spline basis functions from the tensor product space.
...
- `E1::Tuple{SparseMatrixCSC{Float64,Int},SparseMatrixCSC{Float64,Int}}`: The one form
    extraction operators.
- `E2::SparseMatrixCSC{Float64,Int}`: The two form extraction operator.
"""
function build_polar_extraction_operators(
    polar_config::Int,
    degenerate_control_points::NTuple{2, Matrix{Float64}},
    num_basis_p::Int,
    num_basis_r::Int;
    ...
    if build_forms[1]
        E0 = SparseArrays.blockdiag(
            E0_1, SparseArrays.spdiagm(ones(max(0, num_0_forms - 6))), E0_2
        )
    ...
    if build_forms[2]
        ...
        E1_v = SparseArrays.sparse(
            E1_v_r, E1_v_c, E1_v_v, num_1_forms, num_basis_p * (num_basis_r - 1)
        )
        ...
        E1_h = SparseArrays.sparse(
            E1_h_r, E1_h_c, E1_h_v, num_1_forms, num_basis_p * num_basis_r
        )
    ...
    if build_forms[3]
        ...
        # assembly
        E2 = SparseArrays.sparse(
            E2_r, E2_c, E2_v, num_2_forms, num_basis_p * (num_basis_r - 1)
        )
...
# Returns
- `barycentric_coordinates::Tuple{Matrix{Float64},Matrix{Float64}}`: The barycentric
    coordinates.
"""
function barycentric_coordinates_(
    points::NTuple{2, Matrix{Float64}}, trix::Vector{Float64}, triy::Vector{Float64}
)
    L = hcat(trix, triy, ones(3))
    return (
        hcat(zeros(size(points[1])), ones(size(points[1], 1))) / L,
        hcat(points[2] .- points[1], ones(size(points[1], 1))) / L,
    )
end
