...
!!! tip "You can make cool Admonitions boxes"
    These can highlight some of the important stuff.
...
```@meta
CurrentModule = Mantis.ElementLocalBases
```
...
I just found out that we can also combine the docstrings from functions 
...
one big block. We can work on the best way to present things.