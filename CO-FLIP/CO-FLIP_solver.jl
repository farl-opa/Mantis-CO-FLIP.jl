...
struct Domain{F0, F1, F2, G, Q, MH, NH, MM, HF, KR0, KR0F, GG, M1F, M0M, M0F, OBS}
    R0::F0
    R1::F1
    R2::F2
    geo::G
...
Base.@kwdef struct SimulationConfig
    ...
    #             symbolically as `★(d(u_phys_form))`. Exact but differentiation
    #             amplifies high-frequency noise in the projected velocity.
    #   :weak   — Galerkin/L2 projection of the same relation onto the 0-form
    #             space: solve M⁰ w = -G_R0_R1·u (+ boundary term f, which is
...
function GenerateDomain(
    ...
    # 0-form mass matrix factorisation for the weak (Galerkin/L2) vorticity
    # solve M⁰ w = -G_R0_R1·u. M_R0 is SPD and is *not* Dirichlet-pinned, so a
    # Cholesky factorisation is valid and reused at every output/diagnostic step.
    M_R0_fact = cholesky(Symmetric(M_R0))
    ...
    return Domain(R[1], R[2], R[3], Forms.get_geometry(R[2]), dΩ, nel, p, k,
                  A, A_fact, N, M, M_R0, M_R0_fact, K_R0, K_R0_fact, G_R0_R1, M_fact,
...
"""
Compute the discrete vorticity field as an exportable 0-form.

`u_coeffs` are the raw R1 velocity coefficients; `u_phys_form` is their
L2-projected physical-velocity 1-form (`★(u_h)` projected back onto R1), which
is already available at every call site. `method` selects the discretisation
(see `SimulationConfig.vorticity_method`):

- `:strong` — pointwise exterior-calculus relation `ω = ⋆d⋆u`, returned as the
  symbolic expression `★(d(u_phys_form))`. Exact in the continuum but
  differentiation amplifies high-frequency error in `u_phys_form`, so the field
  can look noisy.

- `:weak` — Galerkin/L2 projection of the same relation onto the H¹-conforming
  0-form space. Solves  M⁰ w = N u + f  with  N = -G_R0_R1 = -(D⁰)ᵀM¹  and
  `u = u_coeffs`. The mass-matrix solve smooths the result. The boundary term
  `fᵢ = ∮_∂Ω tr(φ⁰ᵢ ∧ u_phys) = ∮_∂Ω φ⁰ᵢ (u·t̂) ds` is assembled by
  `assemble_vorticity_boundary_term!` over every non-periodic side (it vanishes
  on periodic sides, so the fully-periodic case reduces to `M⁰ w = -G_R0_R1·u`).
"""
function compute_vorticity_form(
        u_coeffs::AbstractVector, u_phys_form, dom::Domain, method::Symbol)
    if method === :strong
        return ★(Forms.d(u_phys_form))
    elseif method === :weak
        # M⁰ w = N u + f with N = -G_R0_R1; f is the ∂Ω tangential-velocity
        # trace (nonzero only on non-periodic sides — see the boundary helper).
        # Both terms use the *raw* physical-velocity 1-form ★(u_form) (the same
        # velocity G_R0_R1 was assembled against), NOT the L2-projected
        # `u_phys_form`, so the volume and boundary contributions stay
        # IBP-consistent and the result is the exact L2 projection of ⋆d⋆u.
        rhs      = -(dom.G_R0_R1 * u_coeffs)
        u_form   = Forms.build_form_field(dom.R1, Vector{Float64}(u_coeffs); label="u_h")
        assemble_vorticity_boundary_term!(rhs, ★(u_form), dom)
        w_coeffs = Vector{Float64}(dom.R0_Mass_fact \ rhs)
        return Forms.build_form_field(dom.R0, w_coeffs; label="w_h")
    else
        throw(ArgumentError(
            "vorticity_method must be :strong or :weak, got $method"))
    end
end

"""
Add the weak-vorticity boundary term to the RHS vector `f` (in place):

    fᵢ += ∮_∂Ω φ⁰ᵢ (u_phys·t̂) ds

the tangential-velocity line integral that arises from integrating
`⟨w, ε⁰⟩ = ⟨⋆d⋆u, ε⁰⟩` by parts (Leibniz: `ε⁰∧d u_phys = d(ε⁰∧u_phys) -
dε⁰∧u_phys`). `t̂` is the unit boundary tangent oriented counter-clockwise
(Stokes orientation of ∂Ω), so each side contributes the velocity component
along it with the sign below:

    bottom (y=ymin): +u_x      top   (y=ymax): -u_x
    left   (x=xmin): -u_y      right (x=xmax): +u_y

Only non-periodic sides are integrated (a periodic side is interior to the
torus and carries no boundary). The R0 basis values and the physical velocity
are evaluated at the *same* per-edge Gauss–Legendre nodes, so the pointwise
product needs no separate ordering bookkeeping. Sanity check: for a uniform
stream u=(1,0) the bottom (+) and top (-) edge integrals cancel the volume
term `-G·u` exactly, giving w≡0 as required.
"""
function assemble_vorticity_boundary_term!(
        f::AbstractVector{Float64}, vel_form, dom::Domain)
    bc = dom.bc_sides              # (left, right, bottom, top)
    nx, ny = dom.nel
    Lx, Ly = dom.box_size
    dx = Lx / nx
    dy = Ly / ny
    nq = maximum(dom.p) + 1        # exact for the degree-2p edge integrand
    nodes, w = gauss_legendre_01(nq)

    # Accumulate one boundary element's contribution. `vel_axis` selects the
    # physical velocity component aligned with the edge tangent (1→u_x for
    # horizontal edges, 2→u_y for vertical), `ds` is the physical edge length
    # per unit reference coordinate, `sgn` the CCW-orientation sign.
    @inline function accumulate_edge!(eid::Int, pts, vel_axis::Int, ds::Float64, sgn::Float64)
        ϕ_comps, idx_groups = Forms.evaluate(dom.R0, eid, pts)    # Vector{Matrix}, indices
        ϕ      = ϕ_comps[1]                                       # 0-form: 1 component, [nq × nlocal]
        idxs   = idx_groups[1]
        pf, _  = Forms.evaluate_sharp_pushforward(vel_form, eid, pts)
        ut     = vec(sum(pf[vel_axis], dims=2))                   # u·t̂ at each node
        @inbounds for k in eachindex(w)
            c = sgn * w[k] * ds * ut[k]
            for b in eachindex(idxs)
                f[idxs[b]] += c * ϕ[k, b]
            end
        end
        return nothing
    ...
    if bc[3] !== :periodic                                        # bottom, η=0
        pts = Mantis.Points.CartesianPoints((nodes, [0.0]))
        for ei in 1:nx
            accumulate_edge!(ei, pts, 1, dx, +1.0)
        end
    end
    if bc[4] !== :periodic                                        # top, η=1
        pts = Mantis.Points.CartesianPoints((nodes, [1.0]))
        for ei in 1:nx
            accumulate_edge!((ny - 1) * nx + ei, pts, 1, dx, -1.0)
        end
    end
    if bc[1] !== :periodic                                        # left, ξ=0
        pts = Mantis.Points.CartesianPoints(([0.0], nodes))
        for ej in 1:ny
            accumulate_edge!((ej - 1) * nx + 1, pts, 2, dy, -1.0)
        end
    end
    if bc[2] !== :periodic                                        # right, ξ=1
        pts = Mantis.Points.CartesianPoints(([1.0], nodes))
        for ej in 1:ny
            accumulate_edge!(ej * nx, pts, 2, dy, +1.0)
        end
    end
    return f
end

"""Compute energy, circulation, and enstrophy diagnostics. `vorticity_method`
selects how ω is reconstructed for the enstrophy (see `compute_vorticity_form`);
energy and circulation are independent of it."""
function compute_conservation_diagnostics(
        u_coeffs::AbstractVector{T}, dom::Domain;
        vorticity_method::Symbol=:strong) where {T<:Real}
    ...
    ω_h          = compute_vorticity_form(u_coeffs, u_phys_form, dom, vorticity_method)
...
function run_diagnostic_simulation(
    ...
    if save_vtk && !is_restart
        ω_h_0 = compute_vorticity_form(u_coeffs, u_phys_form_init, domain, cfg.vorticity_method)
    ...
    d0 = compute_conservation_diagnostics(u_coeffs, domain; vorticity_method=cfg.vorticity_method)
