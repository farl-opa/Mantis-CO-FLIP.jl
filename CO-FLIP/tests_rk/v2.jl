...
function generate_particles(num_particles::Int, domain::Domain)
    ...
    return Particles(positions, impulses, Vector{Float64}(), Vector{Vector{Int}}(), Vector{Tuple{Float64, Float64}}())
end
...
function build_B_matrix(particles::Particles, domain::Domain)
    ...
    # Estimate non-zeros (adjust multiplier based on B-spline order)
    ...
    for elem_idx in eachindex(particles.particles_in_elements)
        ...
        # Extract local coordinates for batch evaluation
        ...
        # Evaluate basis functions at particle locations
        # Note: nderivatives=0 gives us just the values
        eval_out, dof_indices = FunctionSpaces.evaluate(fes, elem_idx, points_wrapped, 0)
        
        # eval_out structure: [Order][Direction][Component]
        # Order 0 has no Direction index, so it is just [1][Component]
        ...
        for (local_pid_idx, global_pid) in enumerate(p_indices)
            # Retrieve particle volume weight
            # If volume is not initialized, default to 1.0 (though it should be fixed in initialization)
            vol = isempty(particles.volume) ? 1.0 : particles.volume[global_pid]
            sqrt_weight = sqrt(vol)

            for k in 1:num_local_basis
                # Scale the matrix entries by sqrt(μ_p)
                val_x = vals_x_matrix[local_pid_idx, k] * sqrt_weight
                val_y = vals_y_matrix[local_pid_idx, k] * sqrt_weight
                
                # Assemble Sparse Matrix (Rows = 2*Particle_ID, Cols = DOF_ID)
                if abs(val_x) > 1e-15
    ...
    # Dimensions: (2 * Num_Particles) x (Num_Grid_DOFs)
...
function transfer_particles_to_grid_pcg(particles::Particles, domain::Domain)
    # 1. Build weighted B matrix (scaled by sqrt(vol))
    B = build_B_matrix(particles, domain)
    num_particles = length(particles.position)
    
    # 2. Build weighted RHS vector (scaled by sqrt(vol))
    V_p = zeros(Float64, 2 * num_particles)
    for i in 1:num_particles
        vx, vy = particles.impulse[i]
        
        vol = isempty(particles.volume) ? 1.0 : particles.volume[i]
        sqrt_weight = sqrt(vol)
        
        V_p[2*i - 1] = vy * sqrt_weight
        V_p[2*i]     = -vx * sqrt_weight  # Note: Impulse stored as covector/1-form
    end

    # 3. Solve Normal Equations: (B^T B) x = B^T V_p
    # Since B and V_p are both pre-scaled, B^T B is the weighted mass matrix
    b_vec = B' * V_p
    ...
    # Compute diagonal of B^T B explicitly for preconditioner
    # diag(B^T B)_j = sum_i (B_ij)^2
    # Since B is sparse, iterating nonzeros is efficient
    # Note: Mantis/SparseArrays dependent, but general logic:
    for col in 1:num_dofs
        sum_sq = 0.0
        for k in nzrange(B, col)
             sum_sq += vals[k]^2
        end
        diag_vals[col] = sum_sq + 1e-8
    end
...
function coadjoint_step!(particles::Particles, domain::Domain, dt::Float64)
    # ==============================================================================
    # 1. INITIAL GUESS (Explicit Step)
    # ==============================================================================
    
    # A. Transfer Particles to Grid (P2G) - y^{(n)} -> f^{(n)}
    # Note: Pass particle volumes to this function for weighted LS (Eq 15)
    u_grid_n = transfer_particles_to_grid_pcg(particles, domain)
    
    # B. Pressure Projection (get divergence-free grid velocity)
    u_grid_n_h = Forms.build_form_field(domain.R1, u_grid_n)
    
    # We need the pressure correction "tau" to update particles later (Section 6.2)
    # u_div_free = u_star - grad(p)  => grad(p) = u_star - u_div_free
    u_div_free_coeffs_n, pressure_grad_coeffs = project_and_get_pressure(domain, u_grid_n_h)
    
    # ==============================================================================
    # 2. FIXED POINT ITERATION (Algorithm 1)
    # ==============================================================================
    # We iterate to find the velocity field v that satisfies the trapezoidal rule.
    
    # Make a copy of particles for the 'next' state y^{(n+1)}
    particles_next = deepcopy(particles) 
    
    # Initial guess for v is just the current velocity
    v_advect_coeffs = u_div_free_coeffs_n
    
    max_iter = 3 # Usually converges in 3-5 iterations (Section 3.4.2)
    
    for iter in 1:max_iter
        # --- Step 2a: Advect Particles y^{(n)} -> y^{(n+1)} using v_advect ---
        # Reset next state to current state
        particles_next.position .= particles.position
        particles_next.impulse  .= particles.impulse
        
        # Advect using the current guess of v_advect
        # Note: The paper uses RK4 for the action, which you implemented correctly.
        rk4_advect_coadjoint!(particles_next, v_advect_coeffs, domain, dt)
        
        # --- Step 2b: P2G on y^{(n+1)} ---
        # Resort particles to correct elements for P2G
        particle_sorter!(particles_next, domain)
        u_grid_next = transfer_particles_to_grid_pcg(particles_next, domain)
        
        # --- Step 2c: Project & Average ---
        # f* = P(I+(y^n+1))
        u_grid_next_h = Forms.build_form_field(domain.R1, u_grid_next)
        u_div_free_coeffs_next, _ = project_and_get_pressure(domain, u_grid_next_h)
        
        # Update Advection Velocity: v = I( 0.5 * (F(y^n) + F(y^n+1)) )
        v_advect_coeffs = 0.5 .* (u_div_free_coeffs_n .+ u_div_free_coeffs_next)
    end
    
    # ==============================================================================
    # 3. PRESSURE FEEDBACK (Section 6.2)
    # ==============================================================================
    # Crucial FLIP step: Add interpolated pressure force to particles.
    # Without this, particles drift from the divergence-free manifold.
    # u_p <- u_p + I(tau)
    
    apply_pressure_force!(particles_next, pressure_grad_coeffs, domain)

    # ==============================================================================
    # 4. FINALIZE
    # ==============================================================================
    particles.position .= particles_next.position
    particles.impulse  .= particles_next.impulse
    particles.particles_in_elements .= particles_next.particles_in_elements
    particles.canonical_positions   .= particles_next.canonical_positions
    
    return v_advect_coeffs
end

function project_and_get_pressure(hp::Domain, v_h)
    f = d(v_h)
    # Solve Δϕ = ∇⋅v
    u_corr, ϕ_corr = Assemblers.solve_volume_form_hodge_laplacian(hp.R1, hp.R2, f, hp.dΩ)
    ...
    # The pressure gradient (or correction term) is u_corr
    return div_free_coeffs, u_corr.coefficients
end

function apply_pressure_force!(particles::Particles, pressure_grad_coeffs, domain::Domain)
    # In CO-FLIP, we add the pressure gradient (flux form interpolated) 
    # back to the particle IMPULSE.
    
    # Note: 'pressure_grad_coeffs' represent the field I(tau) in the paper.
    
    for i in 1:length(particles.position)
        px, py = particles.position[i]
        
        # Probe the pressure gradient field at particle position
        # We only need the value (0th order derivative), not the gradient of the pressure gradient.
        (grad_p_x, grad_p_y), _ = probe_field_at_point(px, py, pressure_grad_coeffs, domain)
        
        mx, my = particles.impulse[i]
        
        # Update impulse (Eq 84)
        # Note: Check signs based on your projection definition. 
        # Typically v_new = v_star - grad_p.
        # If 'pressure_grad_coeffs' is the computed 'u_corr' (which is grad_p),
        # then typically in FLIP we do: v_particle += (v_grid_new - v_grid_old).
        # v_grid_new - v_grid_old = -grad_p.
        # However, Eq 84 says u <- u + I(tau). In Section 6.2, tau is defined 
        # as f_new - f, which is -grad_p.
        
        # Since u_corr is usually grad_p, we subtract it.
        particles.impulse[i] = (mx - grad_p_x, my - grad_p_y)
    end
end
...
function main()
    ...
    grid_probes = Particles(vec(grid_flat), [(0.0,0.0) for _ in 1:length(grid_flat)], Vector{Float64}(), Vector{Vector{Int}}(), Vector{Tuple{Float64, Float64}}())
